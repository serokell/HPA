...
class Augmentation:
    def __init__(self, side=None, strength=1., simplex_noise=False, simplex_vanishing=True):
        cv2.setNumThreads(0)
        cv2.ocl.setUseOpenCL(False)

        self.simplex_noise = SimplexNoise()
        self.simplex_vanishing = simplex_vanishing
    ...
    def __call__(self, data):
        if self.simplex_vanishing and (data['instances'] is not None):
            colours = data['instances'].max()
            if colours > 0:
                colours = np.random.choice(
                    np.arange(colours) + 1, 
                    size=np.ceil(np.random.random() * colours * .5).astype(np.int), 
                    replace=False
                )
                roi = np.isin(data['instances'], colours)
                if np.random.randint(3):
                    data['image'][roi] = (data['image'][roi] * np.random.random()).astype(np.uint8)
                else:
                    data['image'][roi] = 0
...
#                 CLAHE(clip_limit=2, p=.4),
    ...
    def get_geoometric(self):
        geometric = [
            ShiftScaleRotate(shift_limit=0.0625, scale_limit=(-.1, .1), rotate_limit=45, p=.5),
    ...
    def get_augmentations(self):
        ...
        if self.side is not None:
            transformations.insert(
                0, RandomCrop(self.side, self.side)
            )
...
#     GridDistortion, HueSaturationValue, IAAAdditiveGaussianNoise, GaussNoise, MotionBlur, 
#     MedianBlur, IAAPiecewiseAffine, IAASharpen, IAAEmboss, RandomContrast, RandomBrightness, 
#     Flip, OneOf, Compose, ToGray, InvertImg, HorizontalFlip, RandomCrop
# )
...
#     def get_geoometric(self):
#         geometric = [
#             ShiftScaleRotate(shift_limit=0.0625, scale_limit=(-.35, .1), rotate_limit=45, p=.85),
#             OneOf([
#                 OpticalDistortion(p=0.3),
#                 GridDistortion(p=0.3),
#                 IAAPiecewiseAffine(p=0.3),
