...
from src.modules.bn import ABN, InPlaceABN, InPlaceABNSync
from src.modules.misc import GlobalAvgPool2d
from src.modules.residual import IdentityResidualBlock
...
from src.modules.adaptive_avgmax_pool import adaptive_avgmax_pool2d
...
class CatBnAct(nn.Module):
    def __init__(self, in_chs, activation_fn=nn.ReLU(inplace=True), norm_act=ABN):
        super(CatBnAct, self).__init__()
        self.bn = norm_act(in_chs, eps=0.001)#, activation='relu')
...
class BnActConv2d(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size, stride,
                 padding=0, groups=1, activation_fn=nn.ReLU(inplace=True), norm_act=ABN):
...
class InputBlock(nn.Module):
    def __init__(self, num_init_features, kernel_size=7,
                 padding=3, activation_fn=nn.ReLU(inplace=True), norm_act=InPlaceABN):
        super(InputBlock, self).__init__()
        self.conv = nn.Conv2d(
            3, num_init_features, kernel_size=kernel_size, stride=2, padding=padding, bias=False)
        self.bn = norm_act(num_init_features, eps=0.001)#, activation='relu')
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 ...
                 num_classes=28, test_time_pool=False):
    ...
    def forward(self, x):
        ...
        if not self.training and self.test_time_pool:
            ...
            x = torch.nn.functional.dropout(x, p=config.PARAMS['DROPOUT'])
